// Package store handles all PostgreSQL database operations for the telemedicine service.
// It uses pgxpool for connection pooling — pgx is the standard high-performance
// PostgreSQL driver for Go (similar to how Spring Data works in Java, but lower level).
package store
...
// InitSchema creates the tables and indexes the service needs.
// It uses IF NOT EXISTS so it's safe to run on every startup — it won't
// destroy existing data, it just ensures the structure is there.
//
// Tables:
//   - appointments_stub: temporary local appointment records used in "stub" mode
//     (when appointment-service is not available yet)
//   - sessions: one row per video call session, keyed by appointment_id
//   - session_participants: tracks who joined/left and when
//   - session_events: immutable audit log of everything that happened
